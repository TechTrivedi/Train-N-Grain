  ...
  if (!user) {
    return (
      <div className="glass-panel rounded-3xl p-12 text-center border border-white/10 space-y-6 max-w-lg mx-auto my-12">
        <div className="w-16 h-16 rounded-3xl bg-[#00F0FF]/10 border border-[#00F0FF]/30 flex items-center justify-center text-[#00F0FF] mx-auto">
        ...
        <button
          ...
          className="px-8 py-3.5 rounded-2xl bg-[#00F0FF] text-black font-extrabold text-sm shadow-[0_0_25px_rgba(0, 240, 255,0.4)] hover:scale-105 transition-all"
        >
          Sign In Now
        </button>
      </div>
    ...
    <div className="space-y-12 max-w-6xl mx-auto">
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#00F0FF]/20 border border-[#00F0FF] flex items-center justify-center text-[#00F0FF] font-bold text-2xl shadow-[0_0_20px_rgba(0, 240, 255,0.3)]">
        ...
        <div className="flex items-center gap-3">
          <div className="glass-card p-3 rounded-2xl text-center px-5">
            <div className="text-xl font-bold text-[#00F0FF]">{savedWorkouts.length}</div>
            <div className="text-[10px] text-gray-400 uppercase">Workouts</div>
          </div>
          <div className="glass-card p-3 rounded-2xl text-center px-5">
            <div className="text-xl font-bold text-[#00F0FF]">{savedDiets.length}</div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="font-display text-2xl font-bold text-white flex items-center gap-2">
            <Dumbbell className="w-6 h-6 text-[#00F0FF]" /> Saved Workout Programs
          </h3>
          <button
        ...
        {loadingWorkouts ? (
          <div className="glass-panel rounded-2xl p-8 text-center border border-white/10">
            <RefreshCw className="w-6 h-6 text-[#00F0FF] animate-spin mx-auto mb-2" />
          ...
          <div className="glass-panel rounded-2xl p-8 text-center border border-white/10 space-y-3">
            ...
            <button
              onClick={() => setActiveTab('fitness')}
              className="px-4 py-2 rounded-xl bg-[#00F0FF]/10 border border-[#00F0FF]/30 text-[#00F0FF] text-xs font-bold hover:bg-[#00F0FF]/20 transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {savedWorkouts.map((w) => (
              <motion.div
                key={w.id}
                whileHover={{ y: -3 }}
                className="glass-panel p-5 rounded-2xl border border-white/10 space-y-3 hover:border-[#00F0FF]/40 transition-colors flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <h4 className="font-bold text-white text-base truncate">{w.title}</h4>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#00F0FF] px-2 py-0.5 rounded-full bg-[#00F0FF]/10 border border-[#00F0FF]/20">
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    ...
                    <p>Days Count: <strong className="text-[#00F0FF]">{w.plan?.length || 0} Days</strong></p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/10 mt-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-bold text-[#00F0FF] hover:underline"
                  >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="font-display text-2xl font-bold text-white flex items-center gap-2">
            <Utensils className="w-6 h-6 text-[#00F0FF]" /> Saved Diet Plans
          </h3>
          <button
            ...
            {savedDiets.map((d) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    <p>Daily Target: <strong className="text-[#00F0FF]">{d.plan?.calories || 0} kcal</strong></p>
